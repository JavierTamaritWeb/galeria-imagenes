...
- **Rutas de imágenes relativas distintas en SCSS y HTML (2026-07-08).** Síntoma: imágenes 404 si se copia la misma ruta en ambos sitios. Causa: en CSS las URLs se resuelven relativas al CSS compilado (`dist/css/`). Regla: `$face-images` lleva `../img/...` (relativo al CSS compilado) y son rutas **base sin extensión** — el formato lo añade `image-set()`. (Histórico: hubo `<link rel="preload">` en `src/index.html` con `img/...`; se eliminaron al hacer el componente autocontenido — ver la entrada de preload más abajo. Si se reintrodujeran, irían relativos a `dist/`, no a `dist/css/`.)
...
- **Orden del `<head>` (por prioridad de recursos, no tocar a la ligera):** `charset` + `viewport` primero (el `charset` debe caer en los primeros 1024 bytes); luego los metadatos (`color-scheme`, `description`) y el `<title>`; y por último la hoja de estilos (bloquea render). Al añadir recursos, respetar este orden de prioridad (si se reintrodujera un favicon, iría al final por ser de baja prioridad).
- **El componente es autocontenido: no lleva `<link rel="preload">` de las caras a propósito (2026-07-08).** Las 6 caras ya se declaran con `image-set()` en el `background-image`, así que el CSS es la única fuente de carga y se descargan al aplicarse las reglas. CSS no tiene un "preload" real (es un *resource hint* de HTML/HTTP, no de CSS); el preload sólo adelantaba el arranque ~1 round-trip. Se eliminó porque 6 `<link rel="preload">` acoplarían el `<head>` del sitio anfitrión a los nombres de archivo internos del componente — una fuga de abstracción para algo pensado para empotrarse. Regla: no reintroducir preloads en el componente; si una página concreta quiere el arranque temprano, que añada ella el preload o una cabecera HTTP `Link:`.
- **Estructura semántica:** el bloque raíz es un `<main>` (landmark único de la página) y lleva un `<h1 class="visually-hidden">` como primer hijo — da encabezado y outline al documento sin romper el diseño sin texto. La utilidad `.visually-hidden` (en `base/_global.scss`) usa `position: absolute` + `clip-path`, así que queda fuera de flujo y no afecta al centrado ni a la geometría de la cuadrícula. Los selectores del componente son por **clase** (nunca `:first-child`/`> *` sobre hijos directos del bloque), por eso `<main>`/`<h1>` no rompen nada; mantenerlo así al añadir hijos al bloque.
- **Las caras del cubo se mantienen sin `alt` a propósito:** son fondos CSS decorativos (spans vacíos, arte generativo), no `<img>` con valor informativo — AT ya los ignora. No inyectar texto alternativo inventado; si algún día las fotos tuvieran contenido informativo, replantear el marcado.
- Las imágenes viven en `src/img/` y Gulp las copia a `dist/img/` (tarea `images`, con `encoding: false` — sin él Gulp 5 corrompe los binarios; copia todos los formatos con `src/img/**/*`). Las rutas de las caras viven en **un único sitio**: el mapa `$face-images` (base sin extensión, con `../img/...` relativo al CSS compilado). Ya no hay rutas duplicadas en el HTML — al eliminar los preloads, el `index.html` no referencia ninguna cara.
...
- Los 64 `<div class="...__cube">` del HTML están repetidos literalmente porque no hay JS; `$cube-count` en las variables debe coincidir con ese número. Las caras y el aro activo (`__active-indicator`) son `<span>` vacíos (no `<i>`).
