...
- **Orden del `<head>` (por prioridad de recursos, no tocar a la ligera):** `charset` + `viewport` primero (el `charset` debe caer en los primeros 1024 bytes); luego `<title>`; luego los `<link rel="preload">` de las caras AVIF **antes** de la hoja de estilos, para que el preload scanner los descubra y descargue en paralelo con el CSS (si fueran después del `<link rel="stylesheet">`, no se pedirían hasta parsear el CSS y aplicar `image-set()`); después la hoja de estilos (bloquea render); y el favicon SVG al final por ser de baja prioridad. Al añadir recursos, respetar este orden.
